...
class ServiceLayerController extends Controller
{
      ...
      if ($serviceActionRegistry['auth_required'] * 1) { // service needs token
        if ($this->isAuthorized($serviceActionRegistry['id'])) {
          ...
          return response()->json([
            ...
            'token' => auth(),
            'user' => $this->user(null),
            'token_data' => $this->userTokenData
          ], 401)->header('Content-Type', $this->contentType);;
        }
  ...
  public function requestResource($serviceActionRegistry, $param)
  {
    ...
    try {
      ...
      $response = $client->request('POST', $serviceActionRegistry['base_link'] . '/' . $serviceActionRegistry['link'], [
        'json' => $param
      ]);
      // Decode the JSON response as an associative array
      $result = json_decode($response->getBody()->getContents(), true);

      // Safely access the decoded JSON data
      $request['data'] = $result['data'] ?? null;
      $request['additional_data'] = $result['additional_data'] ?? null;
      $request['debug'] = $result['debug'] ?? null;
